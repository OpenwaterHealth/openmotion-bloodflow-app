...
- **User manual (#371) is Research-only and ships with every release.** `docs/user-manual/open-motion-research-user-manual.md` (+ `img/`, rendered by `build_pdf.py` to the committed `Open-Motion-Research-User-Manual.pdf`). Never add clinical or engineering content or a second manual (public repo; `tests/test_user_manual.py` guards it). When a change is visible to operators, update the manual in the same PR and rerun `python docs/user-manual/build_pdf.py`; the `user-manual.yml` workflow fails a PR that changes the manual's sources without the PDF and warns on UI changes without manual changes. Its cover row `Application version` must match a production tag or `release-build.yml` fails before signing (see `AGENTS.md`). CI renders the PDF as a workflow artifact on every PR/push and attaches it to every tagged release. Screenshots need the bench hardware, so refresh them by hand (`docs/user-manual/README.md`).
...
- CI workflows: `.github/workflows/release-build.yml` (Windows runner, builds .exe + zip on tags / manual dispatch; also the user manual PDF), `user-manual.yml` (every PR/push to `next`/`main`: PDF-with-sources check + PDF artifact, #371) and `hil-tests.yml`, which is **retired** (#571, 2026-09-18): the self-hosted HIL system is no longer in use, the workflow is disabled in GitHub and its `workflow_run` trigger is removed; the file is kept for reference with the steps to bring it back in its header. Nothing runs after a release build any more.
...
| Update the user manual | `docs/user-manual/open-motion-research-user-manual.md` → `python docs/user-manual/build_pdf.py` → commit the PDF with it. Rules in `docs/user-manual/README.md`. |
