...
.\build_and_zip.ps1                     # build + package all 4 artifacts (LOCAL; CI never builds both in one place, #573)
...
| `motion_connector.py` | **~6,800 lines.** Single `MotionConnector` QObject — all UI⇄hardware glue, ~64 signals / ~80 slots. State machine constants around line 252; transitions in `update_state` (~line 1947). |
...
| `%LOCALAPPDATA%`, `%USERPROFILE%` / `$HOME` | `utils/app_paths.py` asks the Windows shell (`SHGetKnownFolderPath`) / the passwd db instead |
...
| `PREFERENCE` | `settings` table in `scans.db` | allowed, persisted as a diff vs compiled | plot bounds/colors/window, masks, theme, `showAxisLabels`, `bviLowPassEnabled`, `writeRawCsv`, `rawCsvDurationSec`, `autoExportCsv` |
...
| `portableMode` | derived | **Derived at launch, not configured** (#546): `true` keeps all writable state (logs, scan data/db with the settings table) next to the exe; `false` keeps it per Windows user under `%LOCALAPPDATA%\Openwater` (#581 — it was the machine-wide `%PROGRAMDATA%\Openwater` until then, which locked every account but the first out of a Clinical install: the scan-db key lives in the per-user Credential Manager, so a second account got `EncryptionKeyMissing` → E-301 at every scan start. Legacy ProgramData data is not migrated). A frozen Windows build is installed when its exe lives in the MSI-registered `HKLM\...\Open-Motion\InstallDir` (`installer/app.wxs`), portable otherwise — `utils/app_paths.py:portable_mode`. The zip and the installer therefore share one exe per variant. `python main.py --portable` only labels a source run. **Ignored on macOS** (#428). |
...
| `autoScalePerPlot` | `false` | Per-plot autoscale (#452): with `autoScale` on, each `PlotCell` fits its own camera's trace (`ScanDataSource.compute_bounds_for_cell`) instead of every cell sharing one 2/98-percentile range per metric. Set from the plot's bottom-right ⋯ popup, whose Scale switch reads `Fixed | Global | Per Plot` (autoscale off / on / on + this flag, #622), **research-only**; `PlotViewer.qml` reads the key straight from `appConfig` and forces it off in clinical. `SettingsModal` no longer mirrors it. **Fits the visible window** (#591): the per-cell bounds cover the same `[tLo, tHi]` the cells draw (`_visibleWindow()`: the last `windowSeconds` up to the live-edge snapshot when following live, else the pinned DVR window), because the whole-scan fit let slow BVI drift balloon a cell's axis 20x past the visible trace within minutes. Any pan/zoom/window-length change refits, coalesced into the 33 ms paint tick (`_requestRefit`), and the autoscale Timer evaluates every 0.5 s in per-plot mode (3 s global, unchanged, still whole-scan). **Smoothing:** the 0.5 s evaluation is noisy (2/98 percentiles of ~5 beats move ~7% of the axis per evaluation), so each cell's range follows the fresh windowed fit with an asymmetric first-order lag — 1 s time constant for a bound moving outward, 3 s inward (`_followRange`, `_perPlotTauUpSec` / `_perPlotTauDownSec`; 10 s inward read as "extremely slow" to tighten on the bench; only the Timer path lags, and only while the window is actually sliding — every window change / new source / mode flip snaps, and so does a Timer evaluation whose window has not moved since the last one, i.e. scan stopped or view paused, so nothing creeps after data stops) — applied directly at each evaluation. **No per-frame glide:** an earlier revision eased the shown range toward the target in the 33 ms paint tick, which kept every cell repainting at 30 Hz whenever the target moved (always, in per-plot mode; 30 paints/s vs 9/s in global mode) and made the whole UI sluggish on the bench — per-plot mode must not add repaints beyond the one its evaluation marks dirty (`test_per_plot_refit_applies_at_once_and_does_not_keep_painting`). Bench session 79 replay: ~3.8% of the axis per evaluation, largest move 19% (adopting every fit: 7% / 56%), tallest beats off-axis ~10% of the time. A deadband-with-persistence was tried first (2026-09-23) and gave rarer but much larger moves plus seconds of clipping; do not go back to it. The 0..1 neutral fit holds the previous target — so **a source change must drop every per-plot range** (`onScanSourceChanged` clears `_cellTargets`, #613): a scan starts with no samples, and before that fix each cell kept the previous scan's range and lagged away from it with the traces off-axis for seconds. While a live scan is younger than the window (`tLo < 0`) outward moves are adopted at once (`growNow`): the fitted set is growing, not sliding. **Cost is flat in the zoom (#614):** the fit runs 32x per evaluation and again on every pan/zoom, and at a 5-10 min zoom it made the whole UI slow. `_percentile_pair` is `np.percentile`'s linear method from one `np.partition` (np.percentile had ~100 us of fixed cost per call), and a window over a minute is subsampled to 1-2 minutes' worth (`_fit_step`, keyed on the window length so it never flips while sliding; one golden-ratio pick per stride block, because a fixed-offset stride aliased a steady pulse to a few phases and missed its peaks). One 16-cell evaluation: ~1.4 ms at 15 s, ~3.7 ms at 10 min (was 6 / 18 ms). Panned back past the in-memory ring, `LiveScanSource._cell_chunks` also reads the cached DB-tail window. |
| `plotViewMode` | `"individual"` | Research plot view (#606), from the Individual \| Average buttons at the top of the plot's bottom-right ⋯ popup (PREFERENCE, applies instantly: mid-scan and in replay). `"average"` draws the clinical layout (one plot per sensor module plus the large readout panels, which follow the BFI/BVI ↔ Mean/Contrast pair) from a per-side average of the module's recorded cameras. **Display-only:** the SDK's `reduced_mode` is NOT used (it would also make `scans.db` keep only the averages); instead research sources derive the average under `cam_id=-1` (`data_sources.derive_side_average_buffers` for past scans / the sample CSV / the live DB-tail window, `LiveScanSource`'s per-capture accumulator live), NaN-aware per `frame_id` like the SDK's `spatial_side_average`, BVI averaged raw then low-passed. Derived buffers carry `_CameraBuffer.derived` so the Individual autoscale (`compute_bounds_for_metric`) ignores them; the Average view fits with `compute_bounds_for_side_average`. Clinical builds never derive (`MotionConnector._derive_side_average`), hide the buttons and ignore the key (`PlotViewer.researchAverage` is gated on `clinicalMode`). Per-camera Connection Lost badges don't show in Average (no per-camera cells), same as clinical. Tests: `tests/test_average_view_mode.py`. |
| `showStatistics` | `false` | Research Statistics pane (#635), the **Statistics Off \| On** switch in the plot's ⋯ popup (PREFERENCE, applies instantly; same gate as View: never in a clinical build or a clinical replay). While on, `components/StatisticsPanel.qml` sits right of the plot grid, the per-cell value labels (and the Average view's side panels) are hidden, and the right-anchored plot overlays shift left by the pane (`PlotViewer._statsInsetPx`). One row per plot the current view draws (cameras / Aggregate pairs / side average), grouped LEFT / RIGHT, with the plotted pair's **Live** value (value_at the live edge, like the cell label) and a **0.5 Hz** first-order low-pass of the same stream; then **LEFT − RIGHT** per opposite pair: camera N ↔ 9 − N (the modules are rotated 180° on the body), Aggregate pair ↔ same pair (L1+8 ↔ R1+8), average ↔ average. Differentials are of the displayed (clamped) numbers. The low-pass is `data_sources.lowpass_last` — the BVI display filter's discretization (`bvi_lpf_alpha`, NaN holds state) evaluated statelessly over the trailing `lowpass_window_sec` (4.6 s at 0.5 Hz, seed weight < 1e-6), so it matches a filter run from scan start and works identically live, in replay and when switched on mid-scan. One batch slot per 100 ms refresh (`ScanDataSource.statistics_at`, ~1 ms for 16 cameras). Display-only. Tests: `tests/test_statistics_pane.py`. |
...
| `autoExportCsv` | `false` | Research-only scan-end export (#598): Settings → Data Output "Auto-export CSV" (hidden on clinical builds, engineering unlock included). When on, each scan's `_CompletionSink` callback writes `data/{scan_id}_{subject}_export.csv` — the same file as History → Export CSV (`materialize_corrected_csv`, `include_quality=True`) — on a worker thread, via a `.partial` temp + rename; toast on success/failure, empty scans skipped. PREFERENCE, so it persists; the scan-end gate re-checks `clinicalMode`. Unit tests: `tests/test_scan_auto_export.py`. |
...
The `dataDirectory` config key controls the root (defaults to cwd if unset). When unset on a frozen build, the default is `~/Library/Application Support/Openwater` on macOS, and on Windows follows `portableMode`: next to the exe (portable zip) or the launching user's `%LOCALAPPDATA%\Openwater` (installer; per user since #581). Any of those falling through to an unwritable location lands on `~/Documents/Open-Motion` as a last resort. Two fixed children live under that root:
...
- **Don't track `docs/superpowers/`** — brainstorming specs, implementation plans, and session/status notes are local process artifacts, not repo deliverables. The directory is gitignored (#539 — it was not before, only `.superpowers/` was); keep it that way and never `git add -f` files under it into a PR.
...
- **SBOM (#545):** each CI run generates a CycloneDX SBOM of the build
  ...
  SDK line equals the pin), so the pin check still gates every rc/prod build.
  **The public workflow no longer publishes it** (#573, Ethan 2026-09-18):
  neither as a workflow artifact nor as a GitHub Release asset, Windows and
  macOS alike, so a Research build's SBOM does not outlive its runner. The
  only SBOM that is kept is the Clinical one, which the private repo
  delivers to the Drive next to the binaries (it describes the same build
  environment recipe, but a separate run). There is no tracked SBOM file.
- **What a tag publishes (#573):** this repo is **public** (release assets, workflow artifacts and Actions logs are world-readable), so `release-build.yml` builds **Research only**: dev tag → unsigned zip + installer; rc tag → **signed** zip + installer; production tag → **signed installer only** (no portable zip); macOS DMG on all three, unchanged. **Clinical is never built, uploaded or attached here.** The private `OpenwaterHealth/openmotion-desktop-app-clinical` repo checks this repo out at the tag, runs the same composite action (`.github/actions/windows-build`, `variants: clinical`) and uploads to a Google Shared Drive: `clinical-prerelease.yml` (dev/rc zip + installer, dev unsigned, rc **signed** for now; started fire-and-forget by the `notify-clinical` job via the `CLINICAL_DISPATCH_TOKEN` secret, so neither build can block the other) and `clinical-release.yml` (**manual only**, production tag on `main`, signed installer, refuses unsigned output). A production tag does nothing on the Clinical side by itself. Because that second caller runs in another repository, the action takes the tag as an **input** and must never read `GITHUB_REF` / `github.sha` (guarded in `tests/test_release_workflow.py`).
- **Code signing (#443, merged 2026-09-17):** **rc and production** tag builds Authenticode-sign the Research onefile exe and Setup bundle (Burn engine signed detached, then the reattached bundle) with the SSL.com EV cert via eSigner CKA (cloud key; `ES_*` org secrets); dev tags and branch pushes build unsigned (signings are metered: 3 per signed build per variant, so 6 per rc tag while Clinical rc builds sign too, and 3 + 3 for a production release and its manual Clinical installer). rc signing of both variants is deliberate for the first releases under #573 and Ethan expects to drop it later to conserve quota. The app MSI inside the bundle is deliberately **not** signed (#569): it never ships on its own, Burn runs it already elevated and verifies it by the hash in its signed manifest; the accepted cost is that an AppLocker / WDAC publisher rule for MSIs would block it. A `workflow_dispatch` with the `sign` input also signs — **ask Ethan before triggering one.** The driver is EV-signed separately, once per driver change, via manual dispatch of the SDK's `driver-msi.yml`, then vendored as `resources/OpenMotionDriver-x64.zip`. Everything funnels through `installer/sign.ps1` on `CODESIGN_THUMBPRINT`. **Before this merged no release build had ever been signed** (the step lived only on the PR branch; 1.5.2 shipped unsigned). The in-app updater pins the same certificate (#544). See [docs/SIGNING.md](docs/SIGNING.md).
...
| Touch laser register defaults | `omotion/data/laser_params.json` in the SDK repo — but loop in firmware/SDK owners first; this is locked baseline data. |
