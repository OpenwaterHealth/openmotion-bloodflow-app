...
- **Code signing + pipeline split (#500/#501):** `release-build.yml` runs on **tags only** (nothing builds without a tag — branch-push builds are gone). dev/rc tags build both portables **and both installers, all unsigned** (0 metered signings; QA validates scan + install flows on these). Production tags (`X.Y.Z`) build both portables + the **signed Research installer** — the only automatic signing (3 signings: exe + engine + bundle; hard-fails without the `ES_*` eSigner secrets). The **signed Clinical installer exists only via manual dispatch of `signed-installers.yml`** on a production tag after QA sign-off: it repacks the tag's QA-validated portable zip into the clinical MSI + Setup bundle (2 signings — the exe is already signed; `installer/sign.ps1` skips validly-signed files), verifies every Authenticode signature, and attaches to the tag's release. **MSIs are deliberately unsigned** (quota trim — never distributed standalone; the signed Burn engine owns the UAC prompt). Total per production release: 5 signings. The Clinical portable is QA-only and never distributed; Research portables stay unsigned. The driver is EV-signed separately, once per driver change, via manual dispatch of the SDK's `driver-msi.yml`, then vendored as `resources/OpenMotionDriver-x64.zip` — never rebuilt/re-signed per app release. Everything funnels through `installer/sign.ps1` on `CODESIGN_THUMBPRINT`. See [docs/SIGNING.md](docs/SIGNING.md).
- CI workflows: `.github/workflows/release-build.yml` (Windows runner, tag pushes only — see the pipeline-split bullet above for what each tag type produces), `signed-installers.yml` (manual dispatch — the signed Clinical installer for an existing production tag), and `hil-tests.yml` (self-hosted Windows runner with Shelly IoT outlet power control, runs after each Build & Release run: release set on production tags, dev set on dev/rc tags).
