...
| `altLaserPulseWidthEnabled` | `false` | Engineering, **experiments only** (#449): separate toggle in the same "Camera & laser settings" area as #446 (deliberately not the camera toggle — camera experiments must not silently change laser emission). While on, `altLaserPulseWidthUsec` (whole µs, 100–2200; UI offers 100 µs steps, SDK default 500) replaces `LaserPulseWidthUsec` in the resolved trigger config on **every** `setTrigger` push, logged at WARNING. No restore path needed — the trigger config re-resolves from `omotion/config.py DEFAULT_TRIGGER_CONFIG` each push, so turning it off is enough. Ignored on a plain clinical build (`clinicalMode && !engineeringMode`). Bench facts: optical pulse is driver-shaped at ~494 µs (longer gates mostly add dark-current integration; shorter gates clip); the stock safety FPGA `PULSE_WIDTH_UL` trips and **latches** above ~1000 µs — the operator disables safety first. |
