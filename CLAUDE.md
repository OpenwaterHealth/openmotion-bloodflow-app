...
| `cq_live_debounce_frames` | `80` | Live **mid-scan** contact-quality debounce (#364): consecutive light-frame evaluations (~40 Hz, so 80 ≈ 2 s) a camera must agree on before the warning modal latches or clears. Config-only, no Settings UI. The dark/ambient path is deliberately undebounced — scheduled darks are ~15 s apart, which is its own debounce. Evaluation lives in the SDK (`omotion/contact_quality.py` → `ContactQualityMonitor`), attached as a scan sink by `startCapture`; the connector is a pure signal adapter. Shares `cq_dark_threshold_per_camera` / `cq_light_threshold_per_camera` / `cq_rolling_avg_window` with the pre-scan check, so preflight and live can't disagree. A fully unlit camera (decoupled fiber, covered sensor) is detected via the pipeline's `low_light_rt` flag rather than a threshold — `DarkCorrectionStage` leaves `mean_dc_rt` NaN for those frames, and treating that NaN as "no data" is what left #364 silent on total signal loss. |
