...
- **Code signing + pipeline split (#500/#501):** automatic tag builds (`release-build.yml`) produce **unsigned portable zips only** — QA artifacts; nothing signs automatically anywhere. Installers exist only via manual dispatch of **`signed-installers.yml`**, which repacks the tag's QA-validated portable zip into the MSI + Setup bundles, signs everything with the SSL.com EV cert via eSigner CKA (cloud key — `ES_*` org secrets), verifies every Authenticode signature, and attaches the bundles to the tag's release (fails hard without the secrets). Signings are metered: 7 per both-variant run, 4 single-variant. **Clinical software ships only as that signed installer**; the Clinical portable is QA-only and never distributed; Research portables stay unsigned. The driver is EV-signed separately, once per driver change, via manual dispatch of the SDK's `driver-msi.yml`, then vendored as `resources/OpenMotionDriver-x64.zip`. Everything funnels through `installer/sign.ps1` on `CODESIGN_THUMBPRINT`. See [docs/SIGNING.md](docs/SIGNING.md).
- CI workflows: `.github/workflows/release-build.yml` (Windows runner, unsigned portable zips on tags / manual dispatch), `signed-installers.yml` (manual dispatch only — signed MSI/Setup bundles for an existing tag), and `hil-tests.yml` (self-hosted Windows runner with Shelly IoT outlet power control, runs after the release build completes).
