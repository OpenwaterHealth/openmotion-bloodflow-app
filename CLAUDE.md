...
| `utils/settings_store.py` | The `settings` table in `scans.db` — where PREFERENCE/STATE keys persist (SQLCipher-encrypted + HMAC on clinical builds). Replaced `app_config.local.json`; a leftover one is imported once (preferences only) and deleted. |
...
| `PREFERENCE` | `settings` table in `scans.db` | allowed, persisted as a diff vs compiled | plot bounds/colors/window, masks, theme, `showAxisLabels`, `writeRawCsv`, `rawCsvDurationSec` |
