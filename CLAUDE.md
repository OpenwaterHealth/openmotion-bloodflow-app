...
| `cooldownEnabled` (+ `cooldownStartTempC` 45, `cooldownHysteresisC` 2, `cooldownPollIntervalSec` 5, `cooldownTimerFallbackSec` 600, `cooldownTauSec` 0, `cooldownAmbientC` 25) | `true` | Thermal cooldown scan-start lockout (#102, `thermal_cooldown.py`): while idle the app polls per-camera die temps (TPM 0x4D2A via `i2c_read_register`) and disables Start until every camera ≤ `cooldownStartTempC` (release at threshold − hysteresis). All-unreadable temps fall back to a post-scan timer, then fail open. `cooldownTauSec > 0` (from the #102 characterization) enables the banner ETA. Engineering-mode Override in the banner. **Thresholds are pre-characterization placeholders — E2/E3 of `investigations/102_thermal_cooldown/` set them.** |
