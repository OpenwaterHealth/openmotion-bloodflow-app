...
| `cooldownEnabled` (+ `cooldownStartTempC` 60, `cooldownHysteresisC` 2, `cooldownPollIntervalSec` 5, `cooldownTimerFallbackSec` 600, `cooldownTauSec` 510, `cooldownAmbientC` 29) | `true` | Thermal cooldown scan-start lockout (#102/#330, `thermal_cooldown.py`): **armed by a scan ending**, the app polls per-camera die temps (TPM 0x4D2A via `i2c_read_register`) and disables Start until every camera ≤ threshold − hysteresis, then disarms. Armed-only because pre-scan TPM reads are garbage (no conversion in standby — #330 E0); post-scan reads verified live ≥45 min. All-unreadable temps fall back to the post-scan timer, then fail open. Values from the #330 DVT-1B characterization: hottest slot steady-states ~97 °C at 40 Hz laser-on (no latch, band ~113–117 °C), worst powered-idle cooling τ 8.5 min (510 s), idle floor ~29 °C → ~6–7 min lockout after a full-length scan. Engineering-mode Override in the banner. |
