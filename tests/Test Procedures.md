...
| `test_reducedmode.py` | 37 | Reduced Mode workflow in four classes (Reduced Mode forced on via `app_config.json`, not the Settings modal): keyboard-driven (05–21), mouse-driven (22–32), Settings feature (33–37: Time Window dropdown × 4, Auto-scale Y-axes ON), and modal-exclusivity detector (38: opens Settings then clicks Start, asserts only one modal is visible — currently expected to FAIL until BloodFlow.qml's reduced-mode Start handler closes existing modals before opening ContactQualityModal). | ~52 min |
